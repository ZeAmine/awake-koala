...
const Films = (props) => {
  ...
  const defProviders = function () {
    ...
    if (providers["results"]) {
      try {
        for (let i in providers["results"]["FR"]["flatrate"][0][
          "provider_name"
        ]) {
          if (
            i !==
            providers["results"]["FR"]["flatrate"][0]["provider_name"].length -
              1
          ) {
            provArr.push(
              providers["results"]["FR"]["flatrate"][0]["provider_name"][i] +
                ", "
            );
          } else {
            provArr.push(
            ...
            <div className="textFilm">
              <div className="poster-placement phone-visibility">
                ...
                <div className="movie-page">
                  <a href={homepage}>Page du film</a>
                </div>
              </div>
